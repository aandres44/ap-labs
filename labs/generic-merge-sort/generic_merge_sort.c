...
int main(int argc, char** argv)
{
    ...
    if(argc < 2) {
        ...
        printf("Usage: ./generic_merge_sort <-n for integers/ blank for strings> <filename (must be .txt)>\n");
        return 0;
    }

    

    if(argc == 2 && strcmp(get_filename_ext(argv[1]), "txt") != 0){
        printf("Error: wrong set of parameters.\n");
        printf("Usage: ./generic_merge_sort <-n for integers/ blank for strings> <filename (must be .txt)>\n");
        return 0;
    }
    ...
    mergeSort((void *) lines, 0, index-1, (int (*)(void*, void*)) (type ? ncmp : strcmp));
